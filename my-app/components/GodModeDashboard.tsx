...
import { UserCircle2, ArrowRight, ShieldAlert, Cpu, Link as LinkIcon, CheckCircle2 } from "lucide-react";
...
export default function GodModeDashboard({ activePayloadId }: Props) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        const res = await fetch(`http://localhost:8000/api/status/${activePayloadId}`);
  ...
  const handleApprove = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:8000/api/approve/${activePayloadId}`, {
        method: "POST",
          ...
          {panel.map((expert: any, idx: number) => {
             ...
             return (
               <div key={idx} className="chat-bubble flex gap-4 max-w-[85%] self-start">
                  <div className="mt-1">
                    <UserCircle2 className={isWarning ? "text-amber-400" : "text-emerald-400"} />
                  </div>
                  <div>
